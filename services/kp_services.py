...
class KPServices:
    ...
    COLUMN_MAPPING_KLAIM = {
        'NO_SERTIFIKAT': ['certificate no', 'certificate_no', 'certificate no.', 'no. certificate', 'no_certificate', 'no certificate', 'no_sertifikat', 'sertifikat_no', 'no. sertifikat', 'sertifikat no', 'no sertifikat'],
        'NO_KLAIM': ['no_klaim', 'no klaim', 'no. klaim', 'claim_no', 'claim no.', 'claim no.', 'claimno'],
        'NAMA DEBITUR': ['insured_name', 'nama tertanggung', 'the insured', 'insured', 'nama_debitur', 'debitur'],
        'COB': ['cob_treaty', 'cob eng', 'cob', 'cob_group', 'class_of_business', 'kode', 'product_name'],
    ...
    REQUIRED_KLAIM_GROUPS = [
        ['certificate no', 'certificate_no', 'certificate no.', 'no. certificate', 'no_certificate', 'no certificate', 'no_sertifikat', 'sertifikat_no', 'no. sertifikat', 'sertifikat no', 'no sertifikat'],
        ['no_klaim', 'no klaim', 'no. klaim', 'claim_no', 'claim no.', 'claim no.', 'claimno'],
    ...
    # PREMI ALLOCATION
    @classmethod
    def process_kp_allocation_premi(cls, main_file, reference_file) -> pd.DataFrame:
        df_claim = cls._read_file(main_file, required_terms=cls.REQUIRED_PREMI_GROUPS)
        df_treaty = cls._read_file(reference_file)

        no_sertifikat = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_PREMI['NO_SERTIFIKAT']).astype(str).str.strip()
        insured_name = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_PREMI['NAMA DEBITUR']).astype(str).str.strip()
        cob_treaty = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_PREMI['COB']).astype(str).str.strip()
        inception = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_PREMI['TANGGAL_AWAL']).astype(str).str.strip()
        expiry = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_PREMI['TANGGAL_AKHIR']).astype(str).str.strip()
        currency = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_PREMI['CURRENCY']).astype(str).str.strip()
        uy_final = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_PREMI['UY_FINAL']).astype(str).str.strip()
        
        quota_share = cls._get_numeric_column(df_claim, cls.COLUMN_MAPPING_PREMI['QUOTA_SHARE'])
        surplus = cls._get_numeric_column(df_claim, cls.COLUMN_MAPPING_PREMI['SURPLUS'])

        calculated_rumus, fallback_rumus = cls._extract_or_build_primary_key(df_claim)

        df_clean = pd.DataFrame({
            'CERTIFICATE NO': no_sertifikat,
            'NAMA DEBITUR': insured_name,
            'COB': cob_treaty,
            'TANGGAL_AWAL': inception,
            'TANGGAL_AKHIR': expiry,
            'CURRENCY': currency,
            'UW YEAR': uy_final,
            'QS': quota_share,
            'SPL': surplus,
        ...
        qs_share = df_joined['multiplied_quota_share'] = df_joined['QS'] * qs_mult
        sp_share = df_joined['multiplied_surplus'] = df_joined['SPL'] * sp_mult

        df_joined['multiplied_komisi_qs'] = df_joined['komisi_qs_per_panel'] * qs_share
        df_joined['multiplied_komisi_sp'] = df_joined['komisi_sp_per_panel'] * sp_share
        ...
        output_cols = [
            'CERTIFICATE NO', 'NAMA DEBITUR', 'COB', 'UW YEAR', 'TANGGAL_AWAL', 
            'TANGGAL_AKHIR', 'CURRENCY', 'QS', 'SPL', 'broker_used', 'security_used', 'qs share per panel', 
            'sp share per panel', 'komisi_qs_per_panel', 'komisi_sp_per_panel', 'multiplied_quota_share', 
    ...
    # KLAIM ALLOCATION
    @classmethod
    def process_kp_allocation_claim(cls, main_file, reference_file) -> pd.DataFrame:
        df_claim = cls._read_file(main_file, required_terms=cls.REQUIRED_KLAIM_GROUPS)
        df_treaty = cls._read_file(reference_file)

        no_sertifikat = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_KLAIM['NO_SERTIFIKAT']).astype(str).str.strip()
        no_klaim = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_KLAIM['NO_KLAIM']).astype(str).str.strip()
        insured_name = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_KLAIM['NAMA DEBITUR']).astype(str).str.strip()
        cob_treaty = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_KLAIM['COB']).astype(str).str.strip()
        inception = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_KLAIM['TANGGAL_AWAL']).astype(str).str.strip()
        expiry = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_KLAIM['TANGGAL_AKHIR']).astype(str).str.strip()
        dol = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_KLAIM['DOL_DATE']).astype(str).str.strip()
        ...
        claim_amount = cls._get_column_value(df_claim, cls.COLUMN_MAPPING_KLAIM['CLAIM_AMOUNT']).astype(str).str.strip()
        ...
        output_cols = [
            'CERTIFICATE NO', 'CLAIM NO', 'NAMA DEBITUR', 'COB', 'UW YEAR', 
            'TANGGAL_AWAL', 'TANGGAL_AKHIR', 'DOL', 'CURRENCY', 'CLAIM_AMOUNT', 
